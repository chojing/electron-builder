...
FTP.prototype.list = function (path, zcomp, cb) {
  ...
  this._dataconn(function (err, sock) {
    ...
    var sockerr; var done = false; var replies = 0; var entries; var buffer = ''
...
FTP.prototype.get = function (path, zcomp, cb) {
  ...
  this._dataconn(function (err, sock) {
    ...
    var sockerr; var started = false; var lastreply = false; var done = false; var source
...
FTP.prototype.mkdir = function (path, recursive, cb) { // MKD is optional
  ...
  if (!recursive) { this._send('MKD ' + path, cb) } else {
    var self = this; var owd; var abs; var dirs; var dirslen; var i = -1; var searching = true
...
FTP.prototype.lastMod = function (path, cb) {
  ...
  this._send('MDTM ' + path, function (err, text, code) {
    ...
    var val = XRegExp.exec(text, REX_TIMEVAL); var ret
...
FTP.prototype._pasv = function (cb) {
  var self = this; var first = true; var ip; var port
...
FTP.prototype._send = function (cmd, cb, promote) {
  ...
  if (cmd !== undefined) {
    if (promote) { this._queue.unshift({ cmd: cmd, cb: cb }) } else { this._queue.push({ cmd: cmd, cb: cb }) }
  }
...
FTP.prototype._reset = function () {
  ...
  if (this._actvSock && this._actvSock.writable) { this._actvSock.end() }
